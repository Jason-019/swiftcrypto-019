...
**抗干扰关键设计：** 中心加权采样（半径 5px，~121 像素）抵抗噪声和轻微模糊；3px cell 间隙平衡数据密度与抗模糊；自适应背景阈值从参考行附近本地采样，暗角 90% 仍可检测；校验和裁剪重试自动纠正尾部 1~3 个误读字符。
...
- Base64 字符映射为 MIDI Note On **velocity（力度）** 值（0~63）
